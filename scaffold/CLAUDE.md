...
- **Adapter must be gated to production** — on `@astrojs/cloudflare` 13+, an unconditional adapter loaded in dev replaces Astro's image service with a workerd `image-transform-endpoint` that calls `env.IMAGES`/`env.ASSETS` runtime bindings. Those bindings aren't configured outside `wrangler dev`, so every `<Image>` / `<Picture>` 404s on `bun run dev`. Always:
  ...
  **The value must be `"./client"`, not `"./dist/client"`.** Astro's schema resolves `build.client` relative to the default `outDir` (`./dist`) — `"./dist/client"` becomes `<root>/dist/dist/client/`, which doesn't match `wrangler.toml`'s `[assets] directory = "./dist"`. The Cloudflare adapter sets `preserveBuildClientDir: true`, so production builds honor `build.client` rather than falling back to `outDir`, making the double-nest break the deploy layout.

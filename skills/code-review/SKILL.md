...
- **Warning** — *Favicon / app-icon / manifest paths uncached*. `public/_headers` missing rules (any `max-age >= 604800`, i.e. ≥1 week) for the favicon family — `/favicon.ico`, `/favicon.svg`, `/favicon-*`, `/apple-touch-icon*`, `/android-chrome-*`, `/mstile-*`, `/manifest.webmanifest`, `/browserconfig.xml`.
...
- **Pass** — `_headers` has explicit `Cache-Control` rules for `/_astro/*` (immutable+1y), favicons/manifests (≥30 days), and `/pagefind/*` (≥1 day). HTML responses can keep Cloudflare's `max-age=0` default — that's appropriate for HTML since the CDN edge cache (`cf-cache-status: HIT`) still serves it without origin round-trips.

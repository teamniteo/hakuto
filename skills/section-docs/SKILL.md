...
- For searching use:
  ...
  ```
- **Cloudflare adapter stays gated to production; pagefind is prod-only.** Verify `astro.config.mjs` keeps `adapter:` wrapped in `process.env.NODE_ENV === "production" ? cloudflare({...}) : undefined` AND has `build.client: "./client"` set. The prod build writes the index to `dist/client/pagefind/`; the explicit `build.client` makes that path stable. The value must be `"./client"` (resolved relative to `outDir`), not `"./dist/client"` — the latter double-nests to `dist/dist/client/`. **Do NOT remove the `NODE_ENV` guard to make pagefind work in dev** — on `@astrojs/cloudflare` 13+, an adapter loaded in dev breaks every `<Image>` / `<Picture>` (workerd image-transform-endpoint needs Cloudflare runtime bindings). Test pagefind search on a deploy preview, not localhost. Reject any `ln -sfn client/pagefind dist/pagefind` postbuild symlink — stale workaround.

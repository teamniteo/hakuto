...
- change local raster `<Picture>` usage to `formats={['webp']}`. `fallbackFormat` is **per-source-format**: omit it on `.png`/`.jpg`/`.jpeg`/`.gif`/`.svg` sources (on `.png` it breaks the build with `ENOENT … dist/_astro/<name>.png`), but **set `fallbackFormat="webp"` on `.webp` sources** — `webp` isn't in Astro's `specialFormatsFallback`, so omitting it ships a PNG fallback
